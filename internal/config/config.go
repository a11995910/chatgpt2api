...
var settingEnvKeys = map[string]string{
	...
	"auto_destroy_accounts_enabled":     "CHATGPT2API_AUTO_DESTROY_ACCOUNTS_ENABLED",
	"auto_destroy_account_minutes":      "CHATGPT2API_AUTO_DESTROY_ACCOUNT_MINUTES",
...
func (s *Store) AutoDestroyAccountsEnabled() bool {
	return util.ToBool(s.settingValue("auto_destroy_accounts_enabled", false))
}

func (s *Store) AutoDestroyAccountMinutes() int {
	return normalizePositiveInt(s.settingValue("auto_destroy_account_minutes", 60), 60)
}
...
func (s *Store) Get() map[string]any {
	...
	data["auto_destroy_accounts_enabled"] = s.AutoDestroyAccountsEnabled()
	data["auto_destroy_account_minutes"] = s.AutoDestroyAccountMinutes()
...
func (s *Store) Update(data map[string]any) (map[string]any, error) {
	...
	if value, ok := next["auto_destroy_account_minutes"]; ok {
		next["auto_destroy_account_minutes"] = normalizePositiveInt(value, 60)
	}
