...
type testAccountConfig struct {
	autoDestroyEnabled bool
	autoDestroyMinutes int
}
...
func (c testAccountConfig) AutoDestroyAccountsEnabled() bool  { return c.autoDestroyEnabled }
func (c testAccountConfig) AutoDestroyAccountMinutes() int {
	if c.autoDestroyMinutes <= 0 {
		return 60
	}
	return c.autoDestroyMinutes
}
...
func TestAddAccountsSetsAutoDestroyDeadlineForNewAccounts(t *testing.T) {
	...
	destroyAt, ok := parseAccountTime(account["auto_destroy_at"])
	if !ok {
		t.Fatalf("auto_destroy_at = %#v, want timestamp", account["auto_destroy_at"])
	}
	after := time.Now().UTC().Add(3*time.Minute + time.Second)
...
func TestStartAutoDestroyWatcherDeletesExpiredAccounts(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	accounts.StartAutoDestroyWatcher(ctx, 20*time.Millisecond)

	deadline := time.Now().Add(time.Second)
	for time.Now().Before(deadline) {
		if accounts.GetAccount("expired-token") == nil {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	if account := accounts.GetAccount("expired-token"); account != nil {
		t.Fatalf("expired account still exists: %#v", account)
	}
