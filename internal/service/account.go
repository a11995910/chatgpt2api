...
type AccountConfig interface {
	...
	AutoDestroyAccountsEnabled() bool
	AutoDestroyAccountMinutes() int
}
...
func (s *AccountService) DeleteAccounts(tokens []string) map[string]any {
	return s.deleteAccounts(tokens, "删除", "删除 %d 个账号", nil)
}

func (s *AccountService) DeleteExpiredAutoDestroyAccounts(now time.Time) map[string]any {
	tokens := s.listExpiredAutoDestroyTokens(now)
	if len(tokens) == 0 {
		return map[string]any{"removed": 0, "items": s.ListAccounts()}
	}
	return s.deleteAccounts(tokens, "自动销毁", "自动销毁 %d 个账号", map[string]any{"source": "auto_destroy"})
}

func (s *AccountService) deleteAccounts(tokens []string, operationType, summary string, details map[string]any) map[string]any {
	...
	if removed > 0 {
		...
		s.logs.Add(fmt.Sprintf(summary, removed), logDetail)
	}
...
func (s *AccountService) StartAutoDestroyWatcher(ctx context.Context, interval time.Duration) {
	if interval <= 0 {
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-timer.C:
				if s.autoDestroyEnabled() {
					s.DeleteExpiredAutoDestroyAccounts(time.Now())
				}
				timer.Reset(interval)
			}
		}
...
func (s *AccountService) autoDestroyEnabled() bool {
	return s != nil && s.config != nil && s.config.AutoDestroyAccountsEnabled() && s.config.AutoDestroyAccountMinutes() > 0
}

func (s *AccountService) newAutoDestroyDeadline() string {
	if !s.autoDestroyEnabled() {
		return ""
	}
	return time.Now().Add(time.Duration(s.config.AutoDestroyAccountMinutes()) * time.Minute).UTC().Format(time.RFC3339)
}

func (s *AccountService) listExpiredAutoDestroyTokens(now time.Time) []string {
	s.mu.Lock()
	defer s.mu.Unlock()
	var out []string
	for _, item := range s.items {
...
func normalizeAccount(item map[string]any) map[string]any {
	...
	if autoDestroyAt := util.Clean(normalized["auto_destroy_at"]); autoDestroyAt != "" {
		normalized["auto_destroy_at"] = autoDestroyAt
	} else {
