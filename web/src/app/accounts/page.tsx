...
function formatAccountTime(value: string | null | undefined, expiredLabel: string) {
  if (!value) {
  ...
  const relative = diffMs > 0 ? `剩余 ${days}d ${hours}h` : expiredLabel;
...
function formatRestoreAt(value?: string | null) {
  return formatAccountTime(value, "已到恢复时间");
}

function formatAutoDestroyAt(value?: string | null) {
  return formatAccountTime(value, "已到销毁时间");
}
...
function AccountsPageContent({ session }: { session: StoredAuthSession }) {
  ...
  const renderAutoDestroyInfo = (account: Account) => {
    const destroy = formatAutoDestroyAt(account.autoDestroyAt);
    return (
      <div className="flex flex-col gap-0.5 text-xs leading-5 text-muted-foreground">
        {destroy.relative ? <span className="font-medium text-foreground">{destroy.relative}</span> : null}
        <span>{destroy.absolute}</span>
      </div>
    );
  };
                  ...
                  <Table className="min-w-[1060px]">
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="w-44">销毁时间</TableHead>
                    ...
                    <TableBody>
                      {currentRows.map((account) => (
                        <TableRow key={account.id} className="text-sm text-muted-foreground">
                          ...
                          <TableCell>{renderAutoDestroyInfo(account)}</TableCell>
                          <TableCell>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                              ...
                              <div className="rounded-lg bg-stone-50 p-2">
                                <div className="text-muted-foreground">销毁</div>
                                <div className="mt-1">{renderAutoDestroyInfo(account)}</div>
                              </div>
                            </div>
                          </div>
                        </div>
