...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    auto_destroy_accounts_enabled: Boolean(config.auto_destroy_accounts_enabled),
    auto_destroy_account_minutes: Math.max(1, Number(config.auto_destroy_account_minutes) || 60),
...
type SettingsStore = {
  ...
  setAutoDestroyAccountsEnabled: (value: boolean) => void;
  setAutoDestroyAccountMinutes: (value: string) => void;
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  setAutoDestroyAccountsEnabled: (value) => {
    set((state) => state.config ? { config: { ...state.config, auto_destroy_accounts_enabled: value } } : {});
  },

  setAutoDestroyAccountMinutes: (value) => {
    set((state) => state.config ? { config: { ...state.config, auto_destroy_account_minutes: value } } : {});
  },
