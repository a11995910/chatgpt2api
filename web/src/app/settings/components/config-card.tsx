...
export function ConfigCard() {
  ...
  const setAutoDestroyAccountsEnabled = useSettingsStore(
    (state) => state.setAutoDestroyAccountsEnabled,
  );
  const setAutoDestroyAccountMinutes = useSettingsStore(
    (state) => state.setAutoDestroyAccountMinutes,
  );
      ...
      <div className="flex flex-col gap-5">
        ...
        <section className={configSectionClassName}>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <Field
              className={cn(
                configFieldClassName,
                "rounded-[12px] border border-border/70 bg-background/75 px-3 py-2",
                !config?.auto_destroy_accounts_enabled && "opacity-60",
              )}
            >
              ...
              <NumberInputWithUnit
                ...
                min={1}
                disabled={!config?.auto_destroy_accounts_enabled}
                value={config?.auto_destroy_account_minutes || ""}
                onChange={setAutoDestroyAccountMinutes}
                placeholder="60"
                unit="分钟"
              />
            </Field>
